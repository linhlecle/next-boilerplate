...
const FILES_TO_PARSE = '+(components|pages)/**/!(*.test|*.stories).{js,jsx}';
...
for (const locale of appLocales) {
  ...
  const translationFileName = `${dirPath}/${locale}.json`;
  try {
...
const extractFromFile = async filename => {
  try {
    ...
    const output = await transform(code, { filename });
...
memoryTask.then(files => {
  ...
  extractTask.then(() => {
    ...
    mkdir('-p', dirPath); // eslint-disable-line

    let localeTaskDone;
    let translationFileName;

    for (const locale of appLocales) {
      translationFileName = `${dirPath}/${locale}.json`;
      localeTaskDone = task(
